# -*- coding: utf-8 -*-
"""
V46 + 行业共振 回测引擎 v49 (用户自有CSV权威口径)
=========================================================
数据: 用户GitHub CSV(收盘价/开盘价后复权) + 新浪中证1000(仅收盘, 干净)
信号: 收盘价 MACD(10,20,9) 金叉 -> T+1 开盘成交; 涨停过滤; V4状态机(真实中证1000)

相对 v46_user_backtest.py 的优化:
1. 预计算 buy_candidates_by_sig (每只信号日 -> 候选个股列表), 多方案复用, 大幅提速。
2. backtest 改为 config 驱动, 支持:
   - 共振模式 (difpos / topk / gcfrac / exclude_weak / difpos_breadth)
   - regime 条件: 仅在特定 regime 启用共振过滤 (bull / bullflat / flat / short)
   - score 行业动量倾斜: 在候选排序中给"行业avgDIF更高"的个股加分 (共振层增强)
3. 保留修复: 卖出链式 reason is None (杜绝现金重复入账)。

目标: 在 avgDIF>0(17.26) 之上继续寻找更优共振用法。
"""
...
USER_DIR = "/workspace/data_user/data_extract/data"
zz1000 = pd.read_parquet('/workspace/data/zz1000_hfq.parquet')  # 新浪 中证1000(仅收盘)
...
import openpyxl
code2ind = {}
wb = openpyxl.load_workbook(f'{USER_DIR}/股票池.xlsx', read_only=True, data_only=True)
ws = wb['股票池']
    ...
    with open(path, encoding='utf-8-sig') as fh:
        ...
        for line in fh:
            ...
            if len(p) == 3:
                ...
                m = code_re.search(p[2])
                if not m: continue
                d[m.group()][p[4]] = float(p[5])
                c, dd, pr = p[0], p[1], p[2][:m.start()]
            else:
                continue
            try:
                d[c][dd] = float(pr)
...
close_data = load_long_csv(f'{USER_DIR}/股票池_收盘价.csv')
open_data  = load_long_csv(f'{USER_DIR}/股票池_开盘价.csv')
print(f"收盘价股票数: {len(close_data)} | 开盘价股票数: {len(open_data)}", flush=True)

zz_dates = set(pd.to_datetime(zz1000.index).strftime('%Y-%m-%d').values)
alld = set()
for c in close_data: alld.update(close_data[c].keys())
for c in open_data:  alld.update(open_data[c].keys())
dates_valid = sorted(d for d in alld if START_DATE <= d <= END_DATE and d in zz_dates)
date_pos = {d: i for i, d in enumerate(dates_valid)}
print(f"交易日: {len(dates_valid)} | {dates_valid[0]}~{dates_valid[-1]}", flush=True)

codes = [c for c in close_data if c in open_data and c in code2ind
         and len(close_data[c]) >= 120 and len(open_data[c]) >= 120]
print(f"同时有收盘+开盘+行业(>=120日): {len(codes)}只", flush=True)
...
for code in codes:
    ...
    cl = [close_data[code][d] for d in d_list]
...
def industry_resonance_pass(s, di, mode, param):
    ...
    if mode == 'exclude_weak': return ind_avg_dif[s][di] >= param
    if mode == 'difpos_breadth': return ind_avg_dif[s][di] > param and ind_gc_frac[s][di] >= 0.05
    return True

# ============ 预计算 每只信号日 -> 候选个股列表 ============
# 候选需满足: 3日内金叉 + bar>0 + close>ma144 (个股筛选); 记录 base_score / T+1开盘价 / 是否涨停 / 行业
print("预计算 buy_candidates_by_sig (每只信号日候选)...", flush=True)
buy_candidates_by_sig = {}  # sig_date -> list[(code, base_score, open_price, is_zt, industry)]
for k in range(len(dates_valid)-1):          # k = 信号日索引, 成交日 = k+1
    sig = dates_valid[k]; cur = dates_valid[k+1]
    cands = []
    for code in codes:
        ii = IND[code]['date_to_idx'].get(sig)
        jj = IND[code]['date_to_idx'].get(cur)
        if ii is None or jj is None: continue
        c_i = IND[code]['close'][ii]
        if c_i <= 0 or math.isnan(c_i): continue
        dif_i = IND[code]['dif'][ii]; dea_i = IND[code]['dea'][ii]
        ma144_i = IND[code]['ma144'][ii]; bar_i = IND[code]['bar'][ii]
        if math.isnan(dif_i) or math.isnan(dea_i) or math.isnan(ma144_i): continue
        if bar_i < 0: continue
        if c_i < ma144_i: continue
        gc_days = -1
        for off in range(3):
            if ii-off < 0: continue
            dn = IND[code]['dif'][ii-off]; dp = IND[code]['dif'][ii-off-1] if ii-off-1 >= 0 else float('nan')
            en = IND[code]['dea'][ii-off]; ep = IND[code]['dea'][ii-off-1] if ii-off-1 >= 0 else float('nan')
        ...
        o_j = open_data[code].get(cur, c_i)
        if o_j <= 0 or math.isnan(o_j): o_j = c_i
        base_score = (dif_i/c_i*100)*0.3 + (bar_i/c_i*100)*0.7
        base_score = base_score * PENALTY.get(gc_days, 0.2)
        limit_p = get_limit_price(c_i, IND[code]['board'])
        is_zt = (o_j >= limit_p)
        cands.append((code, base_score, o_j, is_zt, code2ind.get(code)))
    buy_candidates_by_sig[sig] = cands
print(f"  预计算完成, 信号日数: {len(buy_candidates_by_sig)}", flush=True)

# ============ 回测 (config 驱动) ============
def backtest(cfg):
    mode = cfg.get('mode'); param = cfg.get('param', 0.0)
    regime_set = cfg.get('regime_set')   # None=所有regime都过滤; set=仅这些regime过滤
    tilt = cfg.get('tilt', 0.0)          # 行业动量倾斜权重
    label = cfg.get('label', 'cfg')
    ...
    for date_idx in range(n):
        ...
        if len(portfolio) < max_pos and cash > 50000 and sig is not None:
            ...
            total_nav = nav
            sig_di = date_pos[sig]
            filter_active = (regime_set is None) or (idx_state in regime_set)
            candidates = []
            for (code, base_score, o, is_zt, ind) in buy_candidates_by_sig.get(sig, []):
                if code in portfolio: continue
                if ind is None: continue
                if filter_active and not industry_resonance_pass(ind, sig_di, mode, param): continue
                sc = base_score + (tilt * ind_avg_dif[ind][sig_di] if (filter_active and tilt != 0) else 0.0)
                candidates.append((sc, code, o, is_zt))
            candidates.sort(reverse=True)
            for sc, code, o, is_zt in candidates:
                ...
                cash -= total_cost
                portfolio[code] = {'shares': shares, 'entry_price': o, 'hold_days': 0, 'score': sc, 'max_price': o}
                trades_log.append({'date': cur, 'code': code, 'dir': '买入', 'price': o, 'shares': shares, 'pnl': 0, 'reason': '金叉', 'yr': yr})
                yearly[yr]['buy'] += 1
        nav = cash
        for c, p in portfolio.items():
            i = IND[c]['date_to_idx'].get(cur)
...
if __name__ == '__main__':
    runs = []
    runs.append(backtest({'label': 'baseline(无行业过滤)', 'mode': None}))
    runs.append(backtest({'label': '行业共振-dif>0', 'mode': 'difpos', 'param': 0.0}))
    # —— 非bull 阈值精扫 0.18~0.34 ——
    for thr in [0.18, 0.20, 0.22, 0.25, 0.27, 0.30, 0.32, 0.34]:
        runs.append(backtest({'label': f'共振-avgDIF>{thr}@非bull', 'mode': 'difpos', 'param': thr, 'regime_set': {'flat','short'}}))

    print("\n" + "=" * 110)
    print(" V46 + 行业共振 v49 (用户自有CSV口径: 收盘价信号+T+1开盘, 中证1000=新浪)")
    print("=" * 110)
    hdr = f"{'策略':<26}{'总收益%':>10}{'回撤%':>9}{'收益/回撤':>11}{'买入':>7}{'卖出':>7}{'强卖':>7}{'涨停跳过':>9}"
    print(hdr); print("-" * 110)
    for r in runs:
        print(f"{r['label']:<26}{r['total_return']:>10.2f}{r['max_dd']:>9.2f}{r['ratio']:>11.2f}{r['buy']:>7}{r['sell']:>7}{r['force_sell']:>7}{r['ytb_skip']:>9}")
    print("-" * 110)
    print("逐年收益%:")
    yrk = sorted(runs[0]['yearly'].keys())
    print(f"{'策略':<26}" + "".join(f"{y:>8}" for y in yrk))
    for r in runs:
        print(f"{r['label']:<26}" + "".join(f"{r['yearly'][y]:>8.2f}" for y in yrk))
    print("-" * 110)
    for r in runs:
        print(f"[{r['label']}] 卖出原因: {r['sell_reason']} | 耗时{r['time']:.1f}s")
    with open('/workspace/v46_user_v49_results.json', 'w') as f:
        json.dump(runs, f, ensure_ascii=False, indent=2, default=str)
    print("已保存 /workspace/v46_user_v49_results.json")
