...
print("加载用户自有数据(长表CSV: code,date,price; 收盘价含少量4列含名称/粘连行)...", flush=True)
USER_DIR = "/workspace/data_user/data_extract/data"
zz1000 = pd.read_parquet('/workspace/data/zz1000_hfq.parquet')  # 真实中证1000(新浪后复权)
...
# 稳健解析长表CSV: 3列(code,date,price) / 4列(code,name,date,price) / 6列(两行粘连)
import re
from collections import defaultdict
    ...
    with open(path, encoding='utf-8-sig') as fh:
        ...
        for line in fh:
            ...
            if len(p) == 3:
                ...
                if not m: continue
                d[m.group()][p[4]] = float(p[5])          # B 记录(code,name,date,price)
                c, dd, pr = p[0], p[1], p[2][:m.start()]   # A 记录
            else:
                continue
            try:
                d[c][dd] = float(pr)
...
if __name__ == '__main__':
    ...
    runs.append(backtest('difpos', 0.5, '行业共振-avgDIF>0.5'))
    runs.append(backtest('difpos', 1.0, '行业共振-avgDIF>1.0'))
    runs.append(backtest('difpos_breadth', 0.0, '行业共振-avgDIF>0且金叉占比>=5%'))

    print("\n" + "=" * 100)
    print(" V46 + 行业共振 回测对比 (用户自有CSV口径: 收盘价信号+T+1开盘成交, 中证1000=新浪指数)")
    print("=" * 100)
    ...
    with open('/workspace/v46_user_results.json', 'w') as f:
