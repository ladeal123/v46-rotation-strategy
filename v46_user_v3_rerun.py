...
# 量比计算辅助函数
def calc_vol_ratio(code, signal_date, date_idx, vol_data, dates_valid):
    vd=vol_data.get(code)
    if not vd or not signal_date: return None
    ratios=[]
    for off in range(5):
        idx_v=date_idx-1-off
        if idx_v>=0:
            v_t=vd.get(dates_valid[idx_v])
            if v_t and v_t>0:
                sum5=0; cnt5=0
...
ytb_skip_count=0
sell_vol_skip_count=0
dump_sell_count=0
dead_cross_watch={}
death_confirm_count=0
...
for date_idx in range(n):
    ...
    for code,pos in list(portfolio.items()):
        ...
        # 放量下跌卖出: T日跌幅>3% + 量比>2.0
        if pos['hold_days']>=MIN_HOLD and i>=1:
            prev_c=IND[code]['prices'][i-1]
            if prev_c>0 and not math.isnan(prev_c):
                day_chg=(c_i-prev_c)/prev_c
                if day_chg<=-0.03:
                    vol_r=calc_vol_ratio(code, signal_date, date_idx, vol_data, dates_valid)
                    if vol_r is not None and vol_r>2.0:
                        sells.append((code,pos,o_j,'放量下跌'))
                        dump_sell_count+=1
                        continue
        
        # 死叉: 缩量(量比<0.6)跳过, 加入二次确认观察
        if pos['hold_days']>=MIN_HOLD and i>=1:
            ...
            if not(math.isnan(dif_prev) or math.isnan(dea_prev)):
                if dif_prev>dea_prev and dif_i<=dea_i:
                    vol_r=calc_vol_ratio(code, signal_date, date_idx, vol_data, dates_valid)
                    if vol_r is None or vol_r>=0.6:
                        sells.append((code,pos,o_j,'死叉'))
                    else:
                        if code not in dead_cross_watch:
                            dead_cross_watch[code]=1
                        sell_vol_skip_count+=1
                    continue
    ...
    # 死叉二次确认: 连续5天死叉则卖出
    for code in list(dead_cross_watch.keys()):
        if code not in portfolio:
            del dead_cross_watch[code]; continue
        ind2=IND[code]
        i2=ind2['date_to_idx'].get(signal_date)
        if i2 is None: continue
        dif_i2=ind2['dif'][i2]; dea_i2=ind2['dea'][i2]
        if math.isnan(dif_i2) or math.isnan(dea_i2): continue
        if dif_i2 > dea_i2:
            del dead_cross_watch[code]; continue
        dead_cross_watch[code]+=1
        if dead_cross_watch[code]>=5:
            j2=ind2['date_to_idx'].get(current_date)
            if j2 is not None:
                o_j2=ind2['opens'][j2]
                if o_j2<=0 or math.isnan(o_j2): o_j2=ind2['prices'][i2]
                if o_j2>0:
                    sells.append((code,portfolio[code],o_j2,'死叉确认'))
                    death_confirm_count+=1
                    del dead_cross_watch[code]
...
print("\n"+"="*70)
print(" V46 v62_v20_v3 (量比加分+卖端:0.6跳过+二次确认5天+放量下跌)")
print("="*70)
...
print(f"缩量死叉跳过: {sell_vol_skip_count} | 死叉确认卖出: {death_confirm_count} | 放量下跌卖出: {dump_sell_count}")
print(f"\n卖出原因分布: {dict(sell_reason_counter)}")

result={
    'label':'V46 v62_v20_v3 (量比加分+卖端:0.6跳过+二次确认5天+放量下跌)',
    'final_nav':final_nav,
    ...
    'sell_vol_skip_count':sell_vol_skip_count,
    'death_confirm_count':death_confirm_count,
    'dump_sell_count':dump_sell_count,
    'sell_reason_counter':dict(sell_reason_counter),
}
with open('/workspace/backtest_v46_v62_v20_v3.json','w') as f:
    json.dump(result, f, ensure_ascii=False, indent=2, default=str)
print(f"\n已保存到 /workspace/backtest_v46_v62_v20_v3.json")

with open('/workspace/V46_v62_v20_v3_trades.csv','w', newline='') as f:
