...
for date_idx in range(n):
    ...
    if len(portfolio)<max_pos and cash>50000 and signal_date is not None:
        ...
        for code in IND:
            ...
            # MA144斜率加分: 均线向上加分, 向下减分
            slope_bonus=0.0
            ma144_i=IND[code]['ma144'][i]
            if not math.isnan(ma144_i):
                if i>=5:
                    ma144_5=IND[code]['ma144'][i-5]
                    if not math.isnan(ma144_5) and ma144_5>0:
                        slope=(ma144_i-ma144_5)/ma144_5
                        if slope>0.005: slope_bonus=0.10    # 明显向上 +10%
                        elif slope>0.001: slope_bonus=0.05  # 微向上 +5%
                        elif slope<-0.001: slope_bonus=-0.05 # 向下 -5%
                elif ma144_i>0:
                    slope=(ma144_i-c_i)/c_i
                    if slope>0: slope_bonus=0.10  # 价格高于144日线
            
            # 连续上涨天数过滤: 连涨太多则减分
            consec_bonus=0.0
            if i>=5:
                consec_up=0
                for off in range(1, 11):  # 最多看10天
                    if i-off<0: break
                    p_today=IND[code]['prices'][i-off+1]
                    p_yest=IND[code]['prices'][i-off]
                    if not (math.isnan(p_today) or math.isnan(p_yest) or p_today<=0 or p_yest<=0):
                        if p_today>p_yest: consec_up+=1
                        else: break
                if consec_up>=7: consec_bonus=-0.10   # 连涨7天+ → 减10%
                elif consec_up>=5: consec_bonus=-0.05 # 连涨5天+ → 减5%
                elif consec_up>=3: consec_bonus=-0.02 # 连涨3天+ → 减2%
            
            base_score=(dif_i/c_i*100)*0.3+(bar_i/c_i*100)*0.7 - 0.2*vol10_i
            score=base_score*PENALTY.get(gc_days, 0.2)*(1+vol_bonus+rev_bonus+slope_bonus+consec_bonus)
...
print("\n"+"="*70)
print(" V46 v62_v20_consec (量比+超跌+MA144斜率+连涨过滤)")
print("="*70)
...
result={
    'label':'V46 v62_v20_consec (量比+超跌+MA144斜率+连涨过滤)',
    'final_nav':final_nav,
...
with open('/workspace/backtest_v46_v62_v20_consec.json','w') as f:
    json.dump(result, f, ensure_ascii=False, indent=2, default=str)
print(f"\n已保存到 /workspace/backtest_v46_v62_v20_consec.json")

with open('/workspace/V46_v62_v20_consec_trades.csv','w', newline='') as f:
    w=csv.DictWriter(f, fieldnames=['date','code','dir','price','shares','pnl','reason','yr','state'])
    w.writeheader()
    for t in trades_log: w.writerow(t)
print(f"交易记录: /workspace/V46_v62_v20_consec_trades.csv ({len(trades_log)}笔)")